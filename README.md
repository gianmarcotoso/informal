...
```js
import { useForm, useFormList } from '@polartiybit/informal/react'

function Todos() {
	const [todos, setTodos, todosForm] = useForm([])
	const [todosList, { setList, addItem, removeItem, updateItem }] = useFormList(todosForm, (i) => i.id)

	function handleAddItemFormSubmit(e) {
		e.preventDefault()

		const title = e.target.elements.namedItems('title').value
				...
				{todosList.map((todo) => {
					...
					return (
						<li key={todo.id}>
							...
							<input name="title" type="text" value={todo.text} onChange={onEditTodo} />
