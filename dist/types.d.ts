...
export type PathString<T, D extends number = 8> = [D] extends [never] ? never : T extends readonly (infer U)[] ? IsTraversable<U> extends false ? `${BoundedIndex}` : `${BoundedIndex}` | `${BoundedIndex}.${PathString<U, Prev<D>>}` : IsTraversable<T> extends false ? never : {
    [K in keyof T & string]: IsTraversable<T[K]> extends false ? K : K | `${K}.${PathString<T[K], Prev<D>>}`;
}[keyof T & string];
type SplitPath<S extends string> = S extends `${infer Head}.${infer Rest}` ? [Head, ...SplitPath<Rest>] : [S];
export type PathTuple<T, D extends number = 8> = [D] extends [never] ? never : T extends readonly (infer U)[] ? IsTraversable<U> extends false ? [number] : [number] | [number, ...PathTuple<U, Prev<D>>] | FragmentTuple<T, D> : FragmentTuple<T, D>;
type FragmentTuple<T, D extends number> = IsTraversable<T> extends false ? never : {
    [K in PathString<T, D>]: PathValue<T, SplitPath<K>> extends infer V ? IsTraversable<V> extends false ? [K] : [K] | [K, ...PathTuple<V, Prev<D>>] : [K];
}[PathString<T, D>];
type FlattenPath<P extends readonly PropertyKey[]> = P extends [infer K, ...infer Rest] ? Rest extends readonly PropertyKey[] ? K extends string ? [...SplitPath<K>, ...FlattenPath<Rest>] : [K, ...FlattenPath<Rest>] : [] : [];
export type PathValue<T, P extends readonly PropertyKey[]> = WalkPath<T, FlattenPath<P>>;
type WalkPath<T, P extends readonly PropertyKey[]> = P extends [infer K, ...infer Rest] ? T extends readonly (infer U)[] ? Rest extends readonly PropertyKey[] ? Rest extends [] ? U : WalkPath<U, Rest> : never : K extends keyof T ? Rest extends readonly PropertyKey[] ? Rest extends [] ? T[K] : WalkPath<T[K], Rest> : never : any : T;
